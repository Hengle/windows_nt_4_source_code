call getcopy oicomex.c oicomex oiwh
call getcopy libmain.c oicomex oiwh
call getcopy oicmain.c oicomex oiwh
call getcopy abridge.h oicomex oiwh
call getcopy comex.h oicomex oiwh
call getcopy oicom400.mak oicomex oiwh
call getcopy oicom400.def oicomex oiwh
call getcopy oicomex.h include oiwh
call getcopy oierror.h include oiwh
call getcopy engdisp.h include oiwh
call getcopy oidisp.h include oiwh
call getcopy oifile.h include oiwh
call getcopy oiadm.h include oiwh
call getcopy dllnames.h include oiwh
call getcopy taskdata.h include oiwh
call getcopy jinclude.h include oiwh
call getcopy jpeg_win.h include oiwh
call getcopy jconfig.h include oiwh
call getcopy jmemsys.h include oiwh
call getcopy jversion.h include oiwh
call getcopy jpegdata.h include oiwh

copy ..\lib\oidis400.lib
